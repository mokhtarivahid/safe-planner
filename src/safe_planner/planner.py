...
from . import color
from .pddl import parser as pddlparser
from .planners import runner as external_planner
from .pddl import ast as pddl
from . import compilation


def _safe_rmtree(path):
    """Best-effort removal of ``path`` (file or directory tree).

    Never raises; intentionally quiet so callers can use it during shutdown
    handlers without masking the original error.
    """
    if not path:
        return
    try:
...
def _safe_remove_glob(pattern):
    """Remove every path matching ``pattern`` (e.g. ``<workdir>/*.pddl``)."""
    if not pattern:
        return
...
def cleanup(pwd, planners, verbose=0):
    from .planners.runner import kill_jobs
...
class Planner(object):
    ...
    def __init__(self, domain, problem, planners=['ff'], safe_planner=False, rank=False, alloutcome=False, verbose=False):
        ...
        # store the list of external classical planners (canonical names)
        self.planners = [os.path.basename(planner).lower() for planner in planners]

        # check if the planners are available (known to the registry AND
        # executable is locatable on disk via pddl-solvers)
        from .planners import registry
        for planner in self.planners:
            entry = registry.planner_entry(planner)
            if entry is None:
                print(color.fg_red("\n'{0}' is not a known planner!".format(planner)))
                print(color.fg_yellow("currently these planners are available: ") +
                      str(registry.available_planners()))
                exit()
            if registry.resolve_executable(planner) is None:
                print(color.fg_red("\n" + registry.describe_missing(planner)))
                exit()
        ...
        if len(self.domain.derived_predicates) > 0:
            # FF does not support axioms; redirect to ff-x (its derived-predicate
            # variant).
            self.planners = ['ff-x' if 'ff' in p else p for p in self.planners]

            # Fast-Downward heuristics differ in axiom support: lmcut/cea/cg do
            # not support axioms, while ff/add do. Auto-switch to an
            # axiom-safe profile (unless the user already pinned one via
            # --profile). Other planners (SymK, ENHSP, ...) ignore this.
            from .planners.profiles import profiles as _profiles
            for p in self.planners:
                chosen = _profiles.ensure_axiom_safe(p)
                if chosen is not None:
                    print(color.fg_yellow(
                        "-- derived predicates detected: switching '{}' to profile '{}' (axiom-safe)".format(p, chosen)))
        ...
        while True:
            ...
            if not self.verbose:
                if self.alloutcome_planning_call > 100 and self.alloutcome_planning_call % 100 == 0:
                    _safe_remove_glob(self.working_dir + '*.pddl')
    ...
    def _find_open_terminal_state(self):
        '''finds a non-goal terminal state in self.policy and returns it for expansion

        Linear-scan variant used by the NDP2 algorithm. Kept distinct from
        :meth:`find_open_terminal_state` (a BFS) used by the original SP
        algorithm because the two algorithms rely on slightly different
        traversal orders.
        '''
    ...
    def remove_path(self, state):
        '''
        Iteratively remove ``state`` and every descendant reachable through the
        current policy. Replaces the previous recursion-based implementation
        which depended on ``sys.setrecursionlimit(20000)`` for large policies.
        '''
        ...
        while stack:
            ...
            if step is None:
                continue
            for child in self.apply_step(init=s, step=step):
                if child in self.policy:
                    stack.append(child)
