(define (domain blocks-domain)
  ...
  (:action pick-up
    ...
    :effect (and 
      (probabilistic 3/4 (and (holding ?b1) (clear ?b2) (not (emptyhand)) (not (clear ?b1)) (not (on ?b1 ?b2))))
      (probabilistic 1/4 (and (clear ?b2) (on-table ?b1) (not (on ?b1 ?b2)))))
  )
  ...
  (:action put-on-block
    ...
    :effect (and (probabilistic 3/4 (and (on ?b1 ?b2) (emptyhand) (clear ?b1) (not (holding ?b1)) (not (clear ?b2))))
                 (probabilistic 1/4 (and (on-table ?b1) (emptyhand) (clear ?b1) (not (holding ?b1)))))
  )
  (:action put-down
    :parameters (?b - block)
  ...
  (:action pick-tower
    ...
    :effect (probabilistic 1/10 (and (holding ?b2) (clear ?b3) (not (emptyhand)) (not (on ?b2 ?b3))))
  )
  (:action put-tower-on-block
    :parameters (?b1 ?b2 ?b3 - block)
    :precondition (and (holding ?b2) (on ?b1 ?b2) (clear ?b3))
    :effect (and (probabilistic 1/10 (and (on ?b2 ?b3) (emptyhand) (not (holding ?b2)) (not (clear ?b3))))
                 (probabilistic 9/10 (and (on-table ?b2) (emptyhand) (not (holding ?b2)))))
  )
