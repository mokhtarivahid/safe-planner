...
for d, domain in enumerate(domains):
    ...
    for r, result in enumerate(os.listdir(domain)):
        ...
        x =r%(len(LN_STYLE))
        if not planner == 'SAT':
            axs[i, j].plot(df['file'], df['planning_time'], linestyle=LN_STYLE[x][0], marker=LN_STYLE[x][1], markerfacecolor='none', label=(r'%s'%planner), markersize=16, linewidth=3)
...
fig.text(-0.005, 0.5, 'Planning Time (s)', fontsize=18, va='center', rotation='vertical')
fig.text(0.5, -0.01, 'Problems', fontsize=18, ha='center')

fig.tight_layout(pad=.1)
fig.savefig('plot_time.pdf', bbox_inches='tight')

# plt.show()

# crop the output pdf file
os.system('pdfcrop plot_time.pdf plot_time.pdf')
